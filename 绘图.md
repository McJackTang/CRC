...
# 1. 载入心率序列
hr = np.load(hr_pattern)    # 单位：bpm
hr = normalize(hr)
resp = normalize(resp)  # 呼吸信号归一化
fs = 60                    # 采样率 (Hz)
T_secs = 0.4       # 论文中 T=1 s
T_pts = int(T_secs * fs)
L_secs = H_secs = 2    # 前后各1秒
#hr = hr[:1200]  # 截取一段心率序列
#resp= resp[:1200]  # 截取一段呼吸序列
t_hr2,prsa_hr2, f2, Pxx_norm2 = bprsa_localmean(hr, hr, fs, T_secs, L_secs, H_secs, direction='dec')
plot_bprsa(t_hr2,prsa_hr2,f2,Pxx_norm2,kind='dec',signal='HR',target_signal='HR')
#plot_bprsa(t_hr2,prsa_hr,f2,Pxx_norm2,kind='dec',signal='HR',target_signal='RESP')
t_hr1,prsa_hr1, f1, Pxx_norm1 = bprsa_localmean(hr, resp, fs, T_secs, L_secs, H_secs, direction='dec')
plot_bprsa(t_hr1,prsa_hr1,f1,Pxx_norm1,kind='dec',signal='HR',target_signal='RESP')
